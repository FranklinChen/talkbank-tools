...
use batchalign_app::api::{JobSubmission, LanguageSpec, NumSpeakers, ReleasedCommand};
use batchalign_app::options::{FaEngineName, UtrEngine, WorTierPolicy};
...
async fn align_audio_clip_with_options(
    ...
    lang: &str,
    label: &str,
    options: batchalign_app::options::CommandOptions,
) {
    let Some(session) =
    ...
    assert_eq!(
        info.status,
        batchalign_app::api::JobStatus::Completed,
...
async fn align_untimed_audio_clip_with_options(
    ...
    label: &str,
    options: batchalign_app::options::CommandOptions,
) -> (batchalign_app::api::JobInfo, Vec<String>) {
    let Some(session) =
        require_live_direct(InferTask::Fa, "Direct session does not support FA infer").await
    else {
...
async fn align_untimed_audio_clip_with_options_detailed(
    ...
    batchalign_app::api::JobInfo,
    batchalign_app::store::JobDetail,
) {
    let Some(session) =
        require_live_direct(InferTask::Fa, "Direct session does not support FA infer").await
    else {
...
async fn align_server_untimed_audio_clip_with_options(
    ...
    batchalign_app::api::JobInfo,
    batchalign_app::api::JobResultResponse,
    Vec<String>,
) {
    let Some(server) =
        require_live_server(InferTask::Fa, "live server does not support FA infer").await
...
async fn align_yue_untimed_rev_utr_fails_cleanly() {
    ...
    assert_eq!(
        info.status,
        batchalign_app::api::JobStatus::Failed,
