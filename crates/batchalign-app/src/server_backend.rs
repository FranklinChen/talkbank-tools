...
mod tests {
    ...
    use crate::api::{
        CorrelationId, DisplayPath, LanguageCode3, LanguageSpec, MemoryMb, NumSpeakers,
        ReleasedCommand,
    };
    ...
    /// A job that is `queued` in the DB at bootstrap time must get a `job_task`
    /// spawned for it, so it can progress rather than staying `Queued` forever
    /// and blocking new submissions for the same files.
    ///
    /// Before the fix, `bootstrap_test_server_backend` counted queued jobs and
    /// logged "will be resumed by the configured backend" — but never actually
    /// spawned `job_task`. This regression test fails without the fix.
    #[tokio::test]
    async fn bootstrap_dispatches_queued_jobs_on_startup() {
        let tempdir = tempfile::tempdir().expect("tempdir");
        let layout = crate::config::RuntimeLayout::from_state_dir(tempdir.path().join("state"));
        std::fs::create_dir_all(layout.state_dir()).expect("create state dir");
        ...
        // Pre-seed a queued job directly in the DB — simulates a job that survived
        // from a previous session (startup recovery of a crash-interrupted job, or
        // an unresolved memory-gate requeue from a live daemon).
        let filenames = vec!["sample.cha".to_string()];
        let has_chat = filenames.iter().map(|_| true).collect();
        db.insert_job(&NewJobRecord {
            job_id: "queued-at-startup".to_string(),
            correlation_id: "queued-at-startup".to_string(),
            ...
            source_dir: tempdir.path().to_string_lossy().into_owned(),
        ...
        .await
        .expect("insert pre-seeded queued job");
        ...
        // `Some(MemoryMb(1))` effectively disables the gate so this
        // test job reaches `Running` immediately without a memory-
        // pressure requeue. `None` would fall through to the
        // tier-derived headroom and could pressure-stall on smaller
        // hosts.
        let config = crate::config::ServerConfig {
            memory_gate_mb: Some(MemoryMb(1)),
            ..crate::config::ServerConfig::default()
        };

        let bootstrap =
            bootstrap_test_server_backend(config, db, engine, tempdir.path().join("jobs"))
                .await
                .expect("bootstrap test backend");

        assert_eq!(
            bootstrap.queued_jobs, 1,
            "pre-seeded queued job should be counted at startup"
        );

        // Give job_task time to pick up the job and call mark_job_running.
        // A correctly fixed bootstrap spawns job_task immediately; even a brief
        // yield is enough for the status to advance past Queued.
        tokio::time::sleep(Duration::from_millis(300)).await;

        let job = bootstrap
            .backend
            .get_job(&JobId::from("queued-at-startup"))
            .await
            .expect("job should still exist in store");

        assert_ne!(
            job.status,
            JobStatus::Queued,
            "queued job loaded at startup must be dispatched — found status {:?}",
            job.status
        );
