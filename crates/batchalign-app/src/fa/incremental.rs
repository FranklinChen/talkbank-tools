...
pub(crate) async fn process_fa_incremental(
    ...
    if fa_params.review_level != batchalign_chat_ops::fa::ReviewLevel::None {
        batchalign_chat_ops::fa::inject_review_tiers(
            &mut chat_file,
            &repair_decisions,
...
mod tests {
    ...
    use batchalign_chat_ops::fa::{FaTimingMode, FaWord, TimeSpan, apply_fa_results};
    ...
    fn test_incremental_path_enforce_monotonicity_strips_backward_timestamp() {
        ...
        let timings = vec![
            vec![Some(batchalign_chat_ops::fa::WordTiming {
                start_ms: 731556,
                end_ms: 733418,
            })],
            vec![Some(batchalign_chat_ops::fa::WordTiming {
