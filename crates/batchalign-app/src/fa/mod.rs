...
pub(crate) async fn run_fa_from_ast(
    mut chat_file: batchalign_chat_ops::ChatFile,
    parse_errors: Vec<batchalign_chat_ops::ParseError>,
    audio: &AudioContext<'_>,
    worker_lang: &crate::api::LanguageCode3,
    services: PipelineServices<'_>,
    fa_params: &FaParams,
    ...
    let repair_decisions = if fa_params.bullet_repair {
        let repair_result = batchalign_chat_ops::fa::repair_bullets(&mut chat_file, false);
    ...
    let monotonicity_decisions = batchalign_chat_ops::fa::enforce_monotonicity(&mut chat_file);

    // 9d. Inject decision provenance tiers (%xalign / %xrev) for all
    //    pipeline decisions that altered the output.
    {
        let mut all_decisions: Vec<batchalign_chat_ops::decisions::DecisionRecord> = Vec::new();
        ...
        batchalign_chat_ops::decisions::strip_decision_tiers(&mut chat_file);
        if !all_decisions.is_empty() {
            batchalign_chat_ops::decisions::inject_decision_tiers(
                &mut chat_file,
                &all_decisions,
