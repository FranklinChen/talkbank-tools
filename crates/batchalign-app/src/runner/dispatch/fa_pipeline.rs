...
struct AlignAudioTask<'a> {
    ...
    file_lang: LanguageCode3,
    audio_path: PathBuf,
    audio_identity: batchalign_chat_ops::fa::AudioIdentity,
...
impl AudioChatTask for AlignAudioTask<'_> {
    ...
    async fn run_attempt(
        ...
        if let Some(ref bt) = before_text {
            let current_text = batchalign_chat_ops::serialize::to_chat_string(&self.chat_file);
...
async fn process_one_fa_file(
    ...
    let (mut chat_file, parse_errors) =
        batchalign_chat_ops::parse::parse_lenient(&fa_parser, &chat_text);
