...
pub async fn start_test_server(python_path: &str) -> (String, tempfile::TempDir) {
    ...
    let config = ServerConfig {
        ...
        port: 0,
        job_ttl_days: 7,
        // Disable the memory gate so test-echo jobs are never blocked waiting
        // for memory availability. The default (2048 MB) causes timeouts in
        // tests that submit multiple files concurrently.
