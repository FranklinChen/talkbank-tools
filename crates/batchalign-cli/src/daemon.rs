...
fn resolve_force_cpu_for_daemon(layout: &RuntimeLayout, cli_force_cpu: bool) -> bool {
    let mut cfg = batchalign_app::config::load_config_from_layout(layout, None).unwrap_or_default();
    if cli_force_cpu {
        cfg.force_cpu = Some(true);
    }
    batchalign_app::host_facts::EffectiveConfig::resolve_from_server_config(&cfg).force_cpu
}
