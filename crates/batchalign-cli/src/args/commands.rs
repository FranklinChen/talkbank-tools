...
pub struct MorphotagArgs {
    ...
    /// Opt out of L2 dispatch for `@s` (code-switched) words. By
    /// default batchalign3 routes `@s` words to the declared
    /// secondary-language Stanza model for real morphological analysis.
    /// Pass `--no-l2-morphotag` to restore the legacy behavior of
    /// emitting `L2|xxx` placeholders instead.
