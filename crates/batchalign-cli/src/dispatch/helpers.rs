        ...
        match client.get_job(server_url, job_id).await {
            Ok(info) => {
                ...
                if let Some(plan) = &info.execution_plan {
                    use batchalign_app::types::execution_plan::ExecutionStage;
...
fn eprint_cancellation_receipt(receipt: &crate::tui::app::CancelledReceipt) {
...
pub(super) fn cancelled_receipt_from(info: &JobInfo) -> Option<crate::tui::app::CancelledReceipt> {
    let source = info.last_cancelled_source.clone()?;
    let at_iso = format_unix_ts_iso(info.last_cancelled_at?.0);
    Some(crate::tui::app::CancelledReceipt {
        source,
