...
use batchalign_app::released_command_supports_paths_mode;
...
fn build_tui_cancel_provenance(
    signal: crate::tui::TuiCancelSignal,
) -> batchalign_app::api::CancellationRequest {
    ...
    if !info.status.is_terminal() {
        if use_tui && std::io::IsTerminal::is_terminal(&std::io::stdout()) {
            ...
            let (cancel_tx, cancel_rx) =
                tokio::sync::oneshot::channel::<crate::tui::TuiCancelSignal>();
