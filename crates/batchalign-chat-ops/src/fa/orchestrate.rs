...
pub fn apply_fa_results(
    ...
    write_wor: bool,
) -> Vec<crate::decisions::DecisionRecord> {
    ...
    crate::decisions::strip_decision_tiers(chat_file);
    ...
    for &utt_idx in &all_utt_indices {
        if let Some(utt) = get_utterance_mut(chat_file, utt_idx) {
            ...
            if words_dropped > 0 {
                decisions.push(crate::decisions::DecisionRecord {
                    line_idx: utt_idx.0,
                    speaker: utt.main.speaker.as_str().to_string(),
                    strategy: crate::decisions::DecisionStrategy::Fa(
                        crate::decisions::FaStrategy::WordsTimingDropped,
                    ),
...
pub fn enforce_monotonicity(chat_file: &mut ChatFile) -> Vec<crate::decisions::DecisionRecord> {
        ...
        match utt.main.content.bullet.as_ref().map(|b| b.timing.start_ms) {
            Some(s) if s < last_start_ms => {
                decisions.push(DecisionRecord::new_and_trace(
                    ...
                    crate::decisions::DecisionStrategy::Monotonicity(
                        crate::decisions::MonotonicityStrategy::TimingStripped,
                    ),
            ...
            && bullet.timing.end_ms > next_start
        {
            ...
            if next_start <= start_ms {
                ...
                decisions.push(DecisionRecord::new_and_trace(
                    ...
                    prev_utt.main.speaker.as_str().to_string(),
                    crate::decisions::DecisionStrategy::Monotonicity(
                        crate::decisions::MonotonicityStrategy::EndClamped,
                    ),
                    format!(
...
/// - `run_fa_from_ast` fast-path return in `batchalign-app/src/fa/mod.rs`
///
...
pub fn strip_wor_from_monotonicity_stripped_utterances(
    ...
    use crate::decisions::{DecisionStrategy, MonotonicityStrategy};
