...
**Status:** Draft
**Last updated:** 2026-05-27 10:14 EDT
...
COMMON OPTIONS:
  ...
  --operator <NAME>
      Operator identifier recorded in --write-override / override
      file entries. Default: $USER.

  --note <TEXT>
      Free-text note recorded in --write-override / override file
      entries (e.g. "Verified by listening; PAR0 is the SLP.").
```
...
the per-speaker Jaccard scores, the chosen mapping, the
`--operator` identifier, an ISO 8601 timestamp, and any
`--note` text. The format is specified in "Override file format"
below.
...
- Not interactive in this release. The command is batch-only:
  succeed, refuse, or fail. The interactive adjudication tooling
  that produces override files lives in a separate command (TBD).
...
chatter speaker-id asr-anonymous.cha \
  ...
  --write-override batch.overrides.toml \
  --operator alice \
  --note "PAR0 is the child by listening" \
  -o asr-labeled.cha
```
...
- Spec entries for the cleaner and the algorithm live in
  ...
  a spec; tests are generated via `make test-gen`.
