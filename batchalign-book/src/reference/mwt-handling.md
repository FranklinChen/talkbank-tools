...
English (`"en"`) was added to `MWT_LANGS` on 2026-04-04 — prior to that,
the English-specific pipeline branch (with MWT GUM) was dead code.
...
**Python-side hint preservation (2026-04-14).** The 2026-04-04 Rust fix
handled MWT correctly in `inject.rs`, but a regression in
`batchalign/inference/_tokenizer_realign.py::_realign_sentence` was
flattening the `(text, True)` tuples to plain strings via a `_conform(tok)`
helper before the Rust char-DP aligner ran. The aligner returned strings,
the strings went back to Stanza's MWT processor, and MWT silently skipped
Range-token expansion for every contraction. `_realign_sentence` now
overlays Stanza's original tuples onto aligner output where lengths
