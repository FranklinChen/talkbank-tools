...
lives in `crates/batchalign-app/src/runner/dispatch/utr.rs`; the core injection
algorithm lives in `crates/batchalign-chat-ops/src/fa/utr.rs`.
...
**Current default (since 2026-03-30):** `auto` always uses `GlobalUtr`. The
experimental two-pass strategy is available via `--utr-strategy two-pass` but
is not production-ready — it has not been validated on enough corpora and was
disabled after alignment regressions were reported on real files.
...
**Strategy selection** (`select_strategy()` in `fa/utr.rs`):
...
Source: strategy selection in `crates/batchalign-chat-ops/src/fa/utr.rs`,
two-pass config and algorithm in `crates/batchalign-chat-ops/src/fa/utr/two_pass.rs`.
...
`batchalign-chat-ops/src/fa/grouping.rs`. Each group maps to one FA worker call.
...
Source: `crates/batchalign-chat-ops/src/fa/grouping.rs`,
...
> Source files: `crates/batchalign-app/src/runner/dispatch/fa_pipeline.rs`,
> `crates/batchalign-app/src/fa/mod.rs`, `crates/batchalign-app/src/fa/transport.rs`,
> `crates/batchalign-chat-ops/src/fa/orchestrate.rs`
...
`is_whisper_model_unavailable()` in `crates/batchalign-app/src/fa/transport.rs`
...
**Source:** `crates/batchalign-chat-ops/src/fa/postprocess.rs` —
...
**Implementation:** `crates/batchalign-app/src/worker/artifacts_v2.rs`
...
| `fa/utr.rs` | `no timing bullet and no estimate, skipping from FA grouping` | Utterance has no bullet and no timed neighbors for interpolation | Inspect file; utterance will be left unaligned |
| `fa/transport.rs` | `FA group has no audio (segment past end of file)` | Group's time window exceeds audio duration | Check audio file length and CHAT bullets |
| `fa/transport.rs` | `Wave2Vec FA hit recoverable target constraint; retrying group with Whisper FA` | Wave2Vec CTC overflow on one group | Informational; Whisper retry follows automatically |
| `fa/transport.rs` | `Whisper FA unavailable … leaving group words unaligned` | Wave2Vec overflow AND worker has no Whisper model | Informational; affected utterances lose word timing but file completes |
| `fa/transport.rs` | `FA error (raw) … category=validation` | Group failed worker inference for a non-CTC reason | Check server log for root cause; file may fail |
| `fa/mod.rs` | `FA cache batch lookup failed` | SQLite cache read error (non-fatal; misses treated as cache misses) | Check disk space and cache file permissions |
| `fa/mod.rs` | `Failed to deserialize cached FA timings` | Cache entry written by a different schema version | Wipe FA cache: `rm ~/Library/Caches/batchalign3/cache.db*` |
...
## Comparison with BA2 (batchalign2 baseline, 2026-01-09)
