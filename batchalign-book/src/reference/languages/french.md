...
French carries no per-language BA3 MWT-override rules as of
...
No French-specific Stanza defects are tracked as of 2026-04-21.
The 3 `missing_mor` flags on the `eng,fra` pair in the
2026-04-21 12:17 L2 eval were produced by the `FRENCH_ELISION_PREFIXES`
splitting hack (see [History](#history)); they are expected to go to
zero on the next eval run once the removal is verified
end-to-end.
