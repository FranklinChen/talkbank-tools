...
**Last updated:** 2026-04-23 20:15 EDT
...
### BA3 mitigation (ACTIVE — landed 2026-04-13)
...
Implementation: `crates/batchalign-chat-ops/src/nlp/invariants/finite_verb_main_clause.rs`.
Dispatcher: `crates/batchalign-chat-ops/src/nlp/invariants/mod.rs`.
Hook point: `crates/batchalign-chat-ops/src/morphosyntax/inject.rs::inject_results`
(call site immediately before `map_ud_sentence`).

### Tests

**Rust unit tests** (14 tests, all GREEN):
`crates/batchalign-chat-ops/src/nlp/invariants/finite_verb_main_clause.rs`
...
### How BA3 tripped on it (2026-04-13 → fixed 2026-04-14)

`batchalign/inference/_tokenizer_realign.py::_realign_sentence` called an
internal `_conform(tok)` helper that flattened tokens to plain strings
before passing them to the Rust char-DP aligner. When the aligner
returned a 1:1 mapping (the common case where no compound-merging was
needed), the hint tuple was gone and Stanza's MWT processor saw only
bare strings. MWT never fired. All English contractions regressed to
single-token morphology.

### BA3 mitigation (ACTIVE — landed 2026-04-14)
...
* **L2 ML-golden tests (4 tests, all GREEN as of 2026-04-14):**
...
See also: `reference/retokenize-analysis.md` and
`reference/ba2-morphotag-retrospective.md` (the retrospective's correction
...
This entry is listed for completeness; it is not the subject of the
2026-04-13 morphotag work but belongs in the same registry.
...
Observed impact: 18 files in CHILDES Finnish Kirjavainen-MPI
produced between 2026-04-11 and 2026-04-14 carried this pattern
in committed state. All were traceable to morphotag runs.
...
### Symptom (Swedish, 2026-04-15)

When the loader requested ``processors="tokenize,pos,lemma,depparse,mwt"``
for Swedish, the Stanza pipeline never finished loading. The Python
worker subprocess printed the bootstrap traceback and exited before
emitting a ``ready`` signal, the Rust dispatcher reported
``Batch infer failed for language group lang=swe``, and every Swedish
file in the batch was failed cleanly by the language-group failure
aggregator (the 2026-04-14 fix held — no silent corruption, but no
``%mor`` either). The 2026-04-15 overnight morphotag run lost
chunk_106 (500 files) to this.
...
processor before requesting it. That arm was added by Ignas Rudaitis
in BA2 commit ``fec893e1`` (2024-10-04) with the commit message
"Avoid requesting unavailable MWT models" — exactly the bug we hit
...
### BA3 fix (ACTIVE — landed 2026-04-15)
...
* **Stanza version:** 1.11.1 (confirmed 2026-04-21)
* **MWT package:** Italian default
* **Failure class:** linguistic-content quality. Stage 3's MWT Range
  reassembly
  (`crates/batchalign-chat-ops/src/nlp/mapping/mod.rs::assemble_mors`)
...
TalkBank CHAT corpora, 2026-04-22):
...
`batchalign-chat-ops`'s `tokenize_postprocessor` hook runs during
...
### BA3 mitigation (ACTIVE — landed 2026-04-23)

**Update 2026-04-23:** BA3 now carries a per-language reconciler
in `crates/batchalign-chat-ops/src/nlp/lang_it.rs` that collapses
...
1. **The `%mor` count invariant is NOT violated by any of these.**
   ...
   [`mor_count_parity_reference_corpus.rs`](https://github.com/TalkBank/)
   test still passes. **Defect 6 is not an injection-gate failure**
...
**Content correction landed 2026-04-23:** the per-language
reconciler in `crates/batchalign-chat-ops/src/nlp/lang_it.rs` takes
...
* **Stanza version:** 1.11.1 (confirmed 2026-04-22)
* **MWT package:** Italian default
...
**Update 2026-04-23:** The per-language reconciler introduced for
Defect 6 also handles this case. The `la` entry in
`IT_MIS_SPLIT_OVERRIDES` in
`crates/batchalign-chat-ops/src/nlp/lang_it.rs` catches the
...
* **Stanza version:** 1.11.1 (confirmed 2026-04-23)
* **MWT package:** Italian default
...
`crates/batchalign-chat-ops/src/nlp/lang_it.rs` carries a second
...
- **Allowlist unit tests** in
  `crates/batchalign-chat-ops/src/nlp/lang_it.rs`.
