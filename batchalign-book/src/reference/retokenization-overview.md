...
**Status:** Current
**Last updated:** 2026-04-23 22:30 EDT
...
that the 2026-04-23 investigation surfaced.
...
- **Output shape:** speaker-attributed utterances ready for CHAT
  assembly.
- **Source:** `crates/batchalign-chat-ops/src/asr_postprocess/mod.rs`
...
- **Source:** `crates/batchalign-chat-ops/src/retokenize/` module
...
- **Reference:** `reference/morphotag-retokenization.md` (primary),
  `reference/retokenize-analysis.md` (postmortem of the 2026-04-04
  / 2026-04-14 regressions), `reference/mwt-handling.md` (MWT
  mechanics foundational to both Preserve and Retokenize modes).
...
## Per-language coverage — the 2026-04-23 audit
...
2. **BA3's Rust per-language overrides** — morphology-level patches
   ...
   Live in `crates/batchalign-chat-ops/src/nlp/lang_{en,fr,ja}.rs`.
...
| HK Cantonese normalization (Python OpenCC) | zh-HK + 31 domain replacements | `asr_postprocess/cantonese.rs` (Rust `zhconv`) |
...
## Current work: 2026-04-23 investigation

A gap investigation is underway. See
`docs/investigations/2026-04-23-retokenization-gap-plan.md` (in
the private workspace) for the full plan. Summary of the phased
approach:
...
- **Phase 2 — Extend the Stanza decision-probe harness** (v2
  landed earlier 2026-04-23): add a token-count `Gold` and new
  `CandidateClass` variants for the absent rule families.
...
## CHAT provenance emission — resolved 2026-04-23

Two questions about whether retokenization should emit CHAT
provenance annotations were resolved during the investigation:
...
### Transcribe-time English orthographic rules (2026-04-23)

Three English orthographic corrections landed in the transcribe
pipeline on 2026-04-23, each as a narrow hook inside the ASR
post-processing stages. They are *not* part of morphosyntax
retokenization — they run at transcribe time, before Stage 6, and
are gated on `lang == "eng"`. Full spec, allowlists, and
probe-verdict citations live in
...
- **Hebrew 1-to-n mismatch** (surfaced 2026-04-23 Tier B
  investigation): Stanza's Hebrew MWT fires under our
  postprocessor, producing 1-to-2 or 1-to-3 UD-word splits for
  prefixed forms. No other probed language shows this. Worth
  a Hebrew-corpus investigation to determine whether downstream
  code actually breaks on these counts; if so, Hebrew may need
  a per-language postprocessor override.

- **Italian Defect 6/7/8 family** — *partially resolved 2026-04-23*.
  A Rust-side reconciler now ships in
  `crates/batchalign-chat-ops/src/nlp/lang_it.rs` covering two
  allowlist families:
...
- `reference/retokenize-analysis.md` — 2026-04-04/14 regression
  postmortem
...
- `architecture/preprocessing-postprocessing.md` — pipeline
  orchestration
- `architecture/dynamic-programming.md` — why morphosyntax
  retokenize uses deterministic span-join rather than character
  DP (intentional simplification)
