...
**Status:** Current
**Last updated:** 2026-04-14 18:03 EDT
...
[Submit-path retries](../architecture/observability.md#submit-path-retries-2026-04-14)
for the sequence diagram and
[Transient accept-gap at job finalization](../architecture/server-known-issues.md#transient-accept-gap-at-job-finalization)
for why retry is load-bearing.
...
- `released_command_supports_paths_mode(command)` is the authoritative
  ...
  `crates/batchalign-app/src/commands/mod.rs` and re-exported from
  `batchalign_app::lib.rs`. It reads each command's `io_profile`
  (`CommandIoProfile` on `CommandWorkflowDescriptor` in
  `crates/batchalign-app/src/commands/spec.rs`) and returns `true` for
  the `PathsModeText` and `PathsModeAudio` variants.
- `is_local_server(url)` at
  `crates/batchalign-cli/src/dispatch/single.rs` returns `true` only for
...
| `morphotag` | `PathsModeText` (2026-04-14) | Server-side runner reads CHAT input from `source_paths` |
| `utseg` | `PathsModeText` (2026-04-14) | Same runner as morphotag |
| `translate` | `PathsModeText` (2026-04-14) | Same runner as morphotag |
| `coref` | `PathsModeText` (2026-04-14) | Same runner as morphotag |
| `compare` | `PathsModeText` (2026-04-14) | Reads main `.cha` + gold `.cha` pair by path |
| `opensmile` | `ContentOnly` | Intentional: kept on content mode this round |

Prior to the 2026-04-14 change, only the five audio-first commands opted
...
before 2026-04-14, now 512 MB) and failed the whole chunk with
[HTTP 413 Payload Too Large](../architecture/server-known-issues.md#413-payload-too-large-on-large-submission-chunks).
...
| **Server read** | Runner opens each `source_paths[i]` directly via `tokio::fs::read_to_string` (see `crates/batchalign-app/src/runner/dispatch/infer_batched.rs:112-141`) | Runner reads the staged copy from `staging_dir/input/<filename>` that the POST handler wrote before returning 202 |
...
sequenceDiagram
    autonumber
    participant CLI as "CLI dispatch<br/>(batchalign-cli/src/dispatch/single.rs)"
    participant Gate as "Gate: supports_paths_mode(cmd)<br/>&& is_local_server(url)<br/>(single.rs:105-107)"
    participant Paths as "CLI paths builder<br/>(batchalign-cli/src/dispatch/paths.rs)"
    participant Content as "CLI content builder<br/>(batchalign-cli/src/dispatch/single.rs:140-201)"
    participant Srv as "POST /jobs handler<br/>(batchalign-app/src/routes/jobs/mod.rs:161)"
    participant Runner as "Runner file read<br/>(batchalign-app/src/runner/dispatch/infer_batched.rs:112)"
...
`crates/batchalign-app/src/commands/spec.rs`,
`crates/batchalign-app/src/commands/mod.rs`,
`crates/batchalign-app/src/routes/jobs/mod.rs`,
