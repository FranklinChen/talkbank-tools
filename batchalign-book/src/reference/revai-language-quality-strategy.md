...
**Status:** Current
**Last updated:** 2026-04-22 20:15 EDT
...
### Observed incident (2026-04-22)

A ~1-minute Malayalam audio sample was submitted to Rev.AI with
`language=ml`. Rev.AI returned 55 text elements comprising:
...
3. The static table carries dated provenance comments, so a successor
   reads `// Added 2026-04-22 …` and understands why each entry exists
   and when it should be re-evaluated.
...
- Static table lives in `crates/batchalign-app/src/revai/preflight.rs`.
- Preflight hook is `validate_language_support()` in
  `crates/batchalign-app/src/types/request.rs`, in the existing Rev.AI
