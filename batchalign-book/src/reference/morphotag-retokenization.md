...
**Status:** Current
**Last modified:** 2026-04-15 11:15 EDT
...
- [`reference/retokenize-analysis.md`](./retokenize-analysis.md) — historical
  debugging notes and postmortems.
- [`reference/stanza-limitations.md`](./stanza-limitations.md) — Defect 2
  (MWT hint preservation) is the root cause of the 2026-04-13 regression
  discussed below.
...
`crates/batchalign-chat-ops/src/morphosyntax/mod.rs`:
...
- Mapped by `map_ud_sentence()` in
  `crates/batchalign-chat-ops/src/nlp/mapping/mod.rs` (line 43).
...
- Driven by `map_ud_sentence_expanded()` (same file, line 92) for the MOR
  side, and by `retokenize_utterance()` in
  `crates/batchalign-chat-ops/src/retokenize/mod.rs` (line 37) for the main
  tier rewrite.
...
flowchart TD
    ...
    Mode -->|"Preserve"| MapMerge["map_ud_sentence()\n(nlp/mapping/mod.rs:43)"]
    MapMerge --> InjectP["inject_morphosyntax()\n(morphosyntax/inject.rs:260)"]
    InjectP --> Serial["to_chat_string()\n(batchalign-chat-ops/src/serialize.rs)"]

    Mode -->|"StanzaRetokenize"| MapExp["map_ud_sentence_expanded()\n(nlp/mapping/mod.rs:92)"]
    MapExp --> Retok["retokenize_utterance()\n(retokenize/mod.rs:37)"]
    Retok --> Rebuild["rebuild_content()\n(retokenize/rebuild.rs)"]
    Rebuild --> InjectR["inject_morphosyntax()\n(morphosyntax/inject.rs:260)"]
    InjectR --> Serial

    Serial --> Out["CHAT file with %mor / %gra"]

    Retok -.->|"on mismatch"| Taint["mark_parse_taint(Main)\n(retokenize/mod.rs:77)"]
```

Note the re-use of `inject_morphosyntax()` at the bottom of both branches:
the tier-insertion machinery is mode-agnostic. Only the upstream MOR
assembly and the conditional main-tier rewrite differ.
...
sequenceDiagram
    ...
    Note over PP,MWT: The 2026-04-13 regression<br/>was: _conform stripped the<br/>tuple, MWT never fired, all<br/>contractions came back as<br/>single tokens.
```

The fix committed on 2026-04-14 is the "overlay" step in
`_realign_sentence()`. When `len(merged) == len(stanza_tokens)` and the
aligner returned a plain string for a token whose original was a tuple
with the same text, we put the tuple back. This is not a cosmetic change:
without it Stanza's MWT processor sees a plain string and silently skips
expansion.
...
| Character-DP aligner | Internal `align()` function from `batchalign.utils.dp` operating on `PayloadTarget`/`ReferenceTarget` lists, called at line 872. | Rust `align_tokens()` exposed via `batchalign_core`, called from `_tokenizer_realign.py:188`. Hirschberg divide-and-conquer (`crates/batchalign-chat-ops/src/dp_align.rs`). |
...
| File | Role |
|---|---|
| `crates/batchalign-chat-ops/src/morphosyntax/mod.rs` | `TokenizationMode` enum; top-level morphosyntax types |
| `crates/batchalign-chat-ops/src/morphosyntax/inject.rs` | Dispatch branch on `TokenizationMode`; calls retokenize or plain inject |
| `crates/batchalign-chat-ops/src/morphosyntax/payloads.rs` | Per-utterance payload collection sent to Python |
| `crates/batchalign-chat-ops/src/retokenize/mod.rs` | `retokenize_utterance()` entry point |
| `crates/batchalign-chat-ops/src/retokenize/mapping.rs` | `build_word_token_mapping()` and deterministic fallback |
| `crates/batchalign-chat-ops/src/retokenize/rebuild.rs` | AST walk that splices Stanza tokens into the `UtteranceContent` |
| `crates/batchalign-chat-ops/src/retokenize/parse_helpers.rs` | Fragment-parser helpers (re-uses tree-sitter fragment parsing) |
| `crates/batchalign-chat-ops/src/retokenize/tests.rs` | 20 unit tests covering mapping, rebuild, and fallback paths |
| `crates/batchalign-chat-ops/src/nlp/mapping/mod.rs` | `map_ud_sentence()` (merge) and `map_ud_sentence_expanded()` (per-component) |
| `crates/batchalign-chat-ops/src/nlp/mapping/helpers.rs` | `assemble_mors()` — clitic-join MOR construction |
| `crates/batchalign-chat-ops/src/nlp/mapping/context.rs` | `MappingContext`: language, MWT lexicon, L2 config |
| `crates/batchalign-chat-ops/src/inject.rs` | `inject_morphosyntax()` — tier insertion, shared by both modes |
...
- **Phrasal-verb MWTs — resolved 2026-04-15.** Cases like
  `wake@s up@s` now produce `verb|wake part|up` with `COMPOUND-PRT`
  GRA deprel via the L2 merge's Priority 0 check
  (`crates/batchalign-chat-ops/src/morphosyntax/l2/merge.rs::resolve_merged_pos_with_context`).
...
- `crates/batchalign-chat-ops/src/retokenize/tests.rs` — mapping
  deterministic / fallback / mixed cases, rebuild walk, diagnostics, and
  taint marking on mismatch.

Rust ML golden tests (in `crates/batchalign-app/tests/ml_golden/golden.rs`):
...
```bash
cargo nextest run -p batchalign-chat-ops retokenize::tests
```
...
- `crates/batchalign-chat-ops/src/morphosyntax/mod.rs` — `TokenizationMode`
  enum definition (lines 25-46).
- `crates/batchalign-chat-ops/src/morphosyntax/inject.rs` — dispatch branch
  on `TokenizationMode` and call into `retokenize_utterance` (lines 89-258).
- `crates/batchalign-chat-ops/src/retokenize/mod.rs` — `retokenize_utterance`
  entry point, `RetokenizeContext`, `mark_parse_taint` (lines 1-94).
- `crates/batchalign-chat-ops/src/nlp/mapping/mod.rs` — `map_ud_sentence`
  (line 43), `map_ud_sentence_expanded` (line 92).
- `crates/batchalign-chat-ops/src/retokenize/tests.rs` — 20 `#[test]`
  entries.
...
- `crates/batchalign-app/tests/ml_golden/golden.rs` — golden test names at
  lines 547 and 959.
