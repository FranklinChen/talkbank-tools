...
**Status:** Current
**Last updated:** 2026-03-30 17:10 EDT

This page documents overlap-aware alignment improvements: what is shipped,
what is proposed, and what validation is still needed.

## Shipped Features (as of 2026-03-18)
...
| Feature | Default | CLI override | Commit |
|---------|---------|-------------|--------|
| Two-pass overlap UTR | **Gated** — requires `--utr-strategy two-pass` | `--utr-strategy two-pass` to enable | `60cf2c0c` |
| Fuzzy word matching (Jaro-Winkler) | Threshold 0.85 | `--utr-fuzzy 1.0` for exact-only | `43503b65` |
| CA marker window narrowing | Enabled (when two-pass active) | `--utr-ca-markers disabled` | `97234cf6` |
| Density-aware fallback | 30% threshold | `--utr-density-threshold` | `97234cf6` |
| Tight buffer for pass-2 | 500ms | `--utr-tight-buffer` | `97234cf6` |
| End-time overlap clamping | Always on | N/A | (2026-03-30) |
| %wor suppression for CA | Auto when `@Options: CA` | `--nowor` for non-CA | (2026-03-30) |

**Defaults were tuned on:** SBCSAE, Jefferson NB, TaiwanHakka, APROCSA.

**Two-pass gated (2026-03-30):** an operator reported alignment regressions on
`Sprott/21SleepingBeauty.cha` (CA transcript, "two timestamps on main line")
and `EHS/14-mot/1223.cha` ("~1000ms overlap between consecutive utterances").
The two-pass strategy was demoted to opt-in (`--utr-strategy two-pass`).
`Auto` now always selects `GlobalUtr`.

**End-time overlap fix (2026-03-30):** `enforce_monotonicity()` now clamps
utterance N's end to utterance N+1's start, fixing the ~1000ms systematic
overlap from UTR's independent per-utterance token range assignment. This
was a general UTR bug, not two-pass-specific.

**CA %wor suppression (2026-03-30):** `@Options: CA` files automatically
skip %wor generation — CA prosodic notation cannot be represented in %wor.

**Experiment baseline:** `experiments/align-regression-2026-03-30/` in this
repo captures align output for both problem files under both strategies.
...
[Shipped Features](#shipped-features-as-of-2026-03-18) above). The
