...
**Status:** Current
**Last updated:** 2026-04-26 12:30 EDT
...
The Malayalam (`mal`) E220 incident on 2026-04-26 happened because
nobody verified that `num2words` had a Malayalam backend before the
language was declared "supported in transcribe-only mode." Whisper
emitted digits, the validator rejected them, the postprocess pipeline
silently passed them through. This page exists to prevent the same
class of mistake.
...
- If MWT is present, the Stanza-induced retokenize path
  (`crates/batchalign-chat-ops/src/retokenize/`) automatically applies.
- Per-language analysis quirks (clitics, compounds, elision) may need a
  `crates/batchalign-chat-ops/src/nlp/lang_<code>.rs` module —
...
`crates/batchalign-app/src/pipeline/transcribe.rs`. The expansion
pipeline is Rust-only (no Python IPC since 2026-04-26) and is NOT
Stanza-gated.

What determines whether digits get spelled out:

- **CJK (`zho`/`cmn`/`jpn`/`yue`)**: handled in Rust by `num2chinese`
  in `crates/batchalign-chat-ops/src/asr_postprocess/num2chinese.rs`.
...
- **All other cases**: per-language `NUM2LANG` table at
  `crates/batchalign-chat-ops/data/num2lang.json`. Built once by
