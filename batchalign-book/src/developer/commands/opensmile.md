...
|-------|----------|----------------|
| CLI args | `crates/batchalign-cli/src/args/commands.rs` — `OpensmileArgs` | Positional input/output dirs, feature-set, lang |
| Command definition | `crates/batchalign-app/src/commands/opensmile.rs` | `CommandDefinition` impl |
| Audio prep | Shared media prep in `crates/batchalign-app/src/runner/` | Converts audio to mono PCM artifact |
| Worker IPC | `batchalign/inference/opensmile.py` — `extract_features()` | Loads openSMILE, returns feature dict |
| CSV writer | `crates/batchalign-app/src/commands/opensmile.rs` | Typed feature map → row-oriented CSV via `csv` crate |
