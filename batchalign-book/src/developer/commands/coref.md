...
|-------|----------|----------------|
| CLI args | `crates/batchalign-cli/src/args/commands.rs` — `CorefArgs` | lang override |
| Command definition | `crates/batchalign-app/src/commands/coref.rs` | `CommandDefinition` impl |
| Coref orchestration | `crates/batchalign-app/src/coref.rs` | Full-document context assembly, worker dispatch, sparse injection |
| Injection | `crates/batchalign-chat-ops/src/coref.rs` | Writes sparse `%xcoref:` tiers |
...
```bash
make test
cargo nextest run -p batchalign-app -E 'test(coref::)'
