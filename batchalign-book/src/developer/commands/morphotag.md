...
|-------|----------|----------------|
| CLI args | `crates/batchalign-cli/src/args/commands.rs` — `MorphotagArgs` | lang, retokenize, skipmultilang, lexicon, l2-morphotag |
| Options builder | `crates/batchalign-cli/src/args/options.rs` — `build_morphotag_options()` | Maps `MorphotagArgs` → `CommandOptions::Morphotag(MorphotagOptions)` |
| Command definition | `crates/batchalign-app/src/commands/morphotag.rs` | `CommandDefinition` impl, pre-validation gate |
| Morphosyntax orchestration | `crates/batchalign-app/src/morphosyntax/` | Cross-file batching, cache lookup, worker dispatch, result injection |
| Batch dispatch | `crates/batchalign-app/src/runner/dispatch/infer_batched.rs` | Pools all files into a single ML call |
| Injection | `crates/batchalign-chat-ops/src/morphosyntax/` | `inject_results()` — writes `%mor`/`%gra` from typed UD annotations |
| Retokenization | `crates/batchalign-chat-ops/src/retokenize/` | Character-level DP for Stanza word splits/merges |
...
"would deadlock" bailout (replaced 2026-04-14 by idle-cross-group
eviction in `worker/pool/eviction.rs`). The typed
[`LanguageGroupFailure`](https://github.com/TalkBank/talkbank-tools/blob/main/crates/batchalign-app/src/morphosyntax/outcomes.rs)
aggregator receives per-group outcomes, and
[`classify_file_for_injection`](https://github.com/TalkBank/talkbank-tools/blob/main/crates/batchalign-app/src/morphosyntax/outcomes.rs)
...
Code: `crates/batchalign-app/src/morphosyntax/outcomes.rs` (pure
aggregator + classifier, 13 tests),
`crates/batchalign-app/src/morphosyntax/dispatcher.rs` (trait boundary
for fake-pool tests),
`crates/batchalign-app/src/morphosyntax/saturation_tests.rs` (3
end-to-end corruption-regression tests).

See also: [Worker-Pool Saturation and Morphosyntax
Safeguards](../../architecture/worker-pool-saturation.md).
...
flowchart TD
    ...
    parse --> clear --> collect --> infer --> inject --> sweep --> serialize

    clear -. "preserves tier position in dependent_tiers" .-> inject
    collect -. "skips only utterances whose Mor tier is non-empty" .-> infer
```

Diagram verified against: `crates/batchalign-chat-ops/src/morphosyntax/inject.rs`,
`crates/batchalign-chat-ops/src/morphosyntax/payloads.rs`,
`crates/batchalign-chat-ops/src/fa/mod.rs`.

### Tier-order preservation (2026-04-14)
...
4. `crates/batchalign-chat-ops/src/fa/mod.rs::add_wor_tier` uses
   `replace_or_add_tier` instead of the old `remove_wor_tier + push`
   sequence, applying the same preservation principle to `%wor`.

**Regression tests** (in `morphosyntax/tests.rs` and `fa/tests.rs`):

- `clear_then_reinject_preserves_tier_order_mor_gra_wor`
- `add_wor_tier_preserves_tier_order_wor_mor_gra`
- A third tier-order test covering the empty-placeholder sweep

### `collect_payloads` empty-placeholder fix (2026-04-14)
...
`morphotag` requires CHAT Level 1 (parseable with valid headers) before
batching. A file with malformed headers would produce mis-keyed cache entries.
...
(`crates/batchalign-chat-ops/src/retokenize/`) using a character-level
...
See [Per-Utterance Language Routing](../reference/per-utterance-language-routing.md).
...
**Module layout** (`crates/batchalign-chat-ops/src/morphosyntax/l2/`):
...
**Dispatch wiring:** `crates/batchalign-app/src/morphosyntax/batch.rs::dispatch_secondary_l2`.
...
[Ungating Decision](../../reference/l2-morphotag-ungating-decision.md)
for the 2026-04-15 evaluation evidence, and
[2026-04-15 evaluation summary](../../reference/l2-eval-runs/2026-04-15/summary.md)
for the raw per-pair numbers.
...
cargo nextest run -p batchalign-chat-ops -E 'test(retokenize::)'
```
