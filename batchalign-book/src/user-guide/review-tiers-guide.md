...
**Status:** Current
**Last modified:** 2026-04-16 15:40 EDT
...
| Module prefix | Source stage | Typical strategies |
|---|---|---|
| `fa:` | Forced alignment (`align`) | `gap_filled`, `boundary_averaged`, `lis_removal`, `words_timing_dropped` |
| `monotonicity:` | Timing sanity (`align`) | `end_clamped`, `timing_stripped` |
| `utr:` | Utterance timing recovery (`align` pre-pass) | `utr_unmatched`, `utr_zero_duration_skipped` |
| `morphosyntax:` | Stanza mapping (`morphotag`) | `mapping_failed`, `retokenization_failed`, `injection_failed`, `nlp_no_sentences` |

`fa:` and `monotonicity:` decisions are produced by
`crates/batchalign-chat-ops/src/fa/orchestrate.rs`. `utr:` decisions come from
`crates/batchalign-chat-ops/src/fa/utr.rs`. `morphosyntax:` decisions come
from `crates/batchalign-chat-ops/src/morphosyntax/inject.rs`.

## Controlling emission: `--review-level`

Every command that emits review tiers accepts `--review-level`:
...
Source: `CliReviewLevel` in `crates/batchalign-cli/src/args/commands.rs`.
...
`crates/batchalign-chat-ops/src/fa/review_tiers.rs`,
`crates/batchalign-chat-ops/src/decisions.rs`,
`docs/pipeline-decision-metadata-design.md`.
...
per-file archive. The design is tracked in
`docs/pipeline-finalize-command-design.md` in the private workspace.
...
| `utr:utr_unmatched` | Untimed utterance could not be matched to any ASR tokens. | Is the transcript different from what was said? Is the audio silent at that point? |
...
Re-running `align` or `morphotag` always strips existing `%x*` tiers before
emitting new ones (see `strip_decision_tiers()` in
`crates/batchalign-chat-ops/src/fa/review_tiers.rs:47-50`). The ratings you
entered are discarded on re-run. To preserve ratings, finalize (once shipped)
or save a copy before re-running.
...
They will, once the harvest tool ships (see
`docs/pipeline-decision-metadata-design.md` in the private workspace). The
