...
**Status:** Current
**Last updated:** 2026-04-17 15:40 EDT
...
# Align a curated re-run list against the fleet server
batchalign3 --server http://net:8001 align --file-list rerun.txt

# Large re-run list: split into batches of 10
bash scripts/align_batch_run.sh -n 10 -s http://net:8001 rerun.txt
```
...
runtime. Source: `crates/batchalign-app/src/runner/dispatch/`.
...
When `--utr-strategy auto` (the default), the strategy is chosen based on
language and whether the file contains overlapping-speech markers.
...
Non-English defaults to `GlobalUtr` because the two-pass strategy requires
accurate ASR timestamps that non-English ASR cannot reliably produce.
Experimental validation on 7 non-English files across 4 languages confirmed
that `GlobalUtr` matches or beats `TwoPassOverlapUtr` for non-English.
English gains: +4.3 pp on SBCSAE, +3.8 pp on Jefferson NB.
...
| `--utr-strategy {auto,global,two-pass}` | `auto` | Overlap strategy: `auto` is language-aware (English + overlaps → two-pass; otherwise global) |
...
**Non-English files should use `--utr-strategy global` explicitly** if you
see timing regressions. The `auto` strategy already does this by default,
but custom `--utr-engine-custom` setups may behave differently.

**For large re-run lists**, split into batches rather than passing thousands
of files at once:
```bash
...
- [Dynamic Programming](../../architecture/dynamic-programming.md) — Hirschberg alignment algorithm
