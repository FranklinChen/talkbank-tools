...
| **Preserve** a replacement through extract→modify→inject round trips | ✅ Yes | `batchalign-chat-ops` content walkers |
...
Domain-aware word extraction in `batchalign-chat-ops/src/extract.rs`
...
|------|-----------|--------------|
| Extraction | `crates/batchalign-chat-ops/src/fa/extraction.rs:26-29` | Sends the *original* word to the FA worker |
| Count | `crates/batchalign-chat-ops/src/fa/mod.rs:367-374` | Counts 1 for the `ReplacedWord` (regardless of replacement word count) |
| Injection | `crates/batchalign-chat-ops/src/fa/injection.rs:76-84` | Consumes 1 cursor slot, sets `replaced.word.inline_bullet` |
| Preservation | `crates/batchalign-chat-ops/src/fa/mod.rs:510` (`collect_existing_fa_word_timings`) | Reads `replaced.word.inline_bullet` |
...
`crates/batchalign-chat-ops/src/fa/tests.rs::test_wor_policy_replacements_use_original_surface()`
...
| Site | File:line | What it does |
|------|-----------|--------------|
| Extraction | `crates/batchalign-chat-ops/src/extract.rs:140-155` | Sends the *replacement* words to Stanza |
| Count | `model.utterance.mor_alignable_word_count()` (delegated to talkbank-model) | Counts replacement-word count, not 1 |
| Injection | `crates/batchalign-chat-ops/src/inject.rs:38-100` | Asserts injected `%mor` item count == count, then injects |
...
- **Generate replacements during retokenization.** When Stanza
  ...
  (`crates/batchalign-chat-ops/src/retokenize/rebuild.rs:77-91`). It
...
- **Generate replacements during ASR retrace detection.** Detected
  ...
  `crates/batchalign-chat-ops/src/build_chat.rs::build_word_utterance`
...
| Concern | File:line |
|---------|-----------|
| Replacement extraction (per-domain branch) | `crates/batchalign-chat-ops/src/extract.rs:140-155` |
| FA extraction (uses original) | `crates/batchalign-chat-ops/src/fa/extraction.rs:26-29` |
| FA count | `crates/batchalign-chat-ops/src/fa/mod.rs:367-374` |
| FA injection | `crates/batchalign-chat-ops/src/fa/injection.rs:76-84` |
| FA preservation | `crates/batchalign-chat-ops/src/fa/mod.rs:510` |
| `%mor` injection (count check) | `crates/batchalign-chat-ops/src/inject.rs:38-100` |
| Retokenize preserves `ReplacedWord` | `crates/batchalign-chat-ops/src/retokenize/rebuild.rs:77-91` |
| Read-only consumption test | `crates/batchalign-chat-ops/src/fa/tests.rs::test_wor_policy_replacements_use_original_surface` |
...
- [CHAT Content Model](./chat-content-model.md) — how `UtteranceContent`
...
- [Content Traversal Patterns](./content-traversal.md) — `walk_words`,
  `WordItem`, and the per-domain extraction primitives.
