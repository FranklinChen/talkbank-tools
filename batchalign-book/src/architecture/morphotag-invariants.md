...
**Status:** Current
**Last updated:** 2026-04-21 18:45 EDT
...
flowchart TD
    ...
    E["Stage 1: Extract<br/>collect_utterance_content(Mor)<br/>(batchalign-chat-ops/extract.rs)"] --> Nlabel
    Nlabel{{"N Mor-alignable words<br/>(counts_for_tier rule)"}}

    Nlabel -->|"N == 0"| NA["MorOutcome::NotApplicable<br/>no %mor produced (correct)"]
    Nlabel -->|"N > 0"| D

    D["Stage 2: Dispatch<br/>tok_ctx.original_words = word_lists<br/>(morphosyntax.py:348)"] --> S
    S["Stanza neural tokenizer<br/>re-tokenizes text, realigning<br/>to word_lists boundaries"] --> P

    P["Stage 3: Project<br/>map_ud_sentence + MWT Range reassembly<br/>(batchalign-chat-ops/nlp/mapping/mod.rs)"] --> M
    M{{"|mors| =?= N"}}
...
`crates/batchalign-chat-ops/src/retokenize/`.)
...
- `crates/batchalign-chat-ops/src/morphosyntax/outcome.rs` — outcome types
- `crates/batchalign-chat-ops/src/inject.rs` — invariant check
- `crates/batchalign-chat-ops/src/morphosyntax/inject.rs` — outcome emission
- `crates/batchalign-chat-ops/src/morphosyntax/payloads.rs` — NotApplicable classification
...
- `crates/batchalign-chat-ops/tests/mor_count_parity_reference_corpus.rs` — Stage 1 tests
