...
audit of fleet jobs.db (2026-04-26) shows no non-English transcribe
job has needed those modes; cardinals (the common case) are covered
for every language in the registry. Add a per-language ordinal/decade
expander if a real corpus surfaces the need.

### Per-language registry (Layer 1 — landed 2026-04-26)

`crates/batchalign-chat-ops/src/asr_postprocess/registry.rs::NUMBER_EXPANDERS`
...
Run: `uv run python scripts/codegen_num2lang.py --output crates/batchalign-chat-ops/data/num2lang.json`
...
| File | Purpose |
|------|---------|
| `crates/batchalign-app/src/pipeline/transcribe.rs` | `stage_asr_postprocess`, `prepare_asr_chunks` (per-word Rust expansion + whitespace split + finalize) |
| `crates/batchalign-chat-ops/src/asr_postprocess/num2text.rs` | `expand_number(word, lang)` — top-level Rust entry; `detect_expansion`; currency/percent/dash helpers; `NUM2LANG` static map |
| `crates/batchalign-chat-ops/src/asr_postprocess/ordinal_year_eng.rs` | `expand_ordinal_eng`, `expand_year_eng`, `expand_decade_eng` (English-only deterministic composition; cross-validated against `num2words` via `data/eng_ordinal_year_fixtures.json`) |
| `crates/batchalign-chat-ops/src/asr_postprocess/num2chinese.rs` | `num2chinese(n, script)` for CJK |
| `crates/batchalign-chat-ops/src/asr_postprocess/registry.rs` | `NumberExpander` enum + `NUMBER_EXPANDERS` per-language registry |
| `crates/batchalign-chat-ops/data/num2lang.json` | Per-language Rust tables (43 codegenned + 4 hand-curated) |
| `crates/batchalign-chat-ops/data/eng_ordinal_year_fixtures.json` | Cross-validation fixtures for `ordinal_year_eng` |
...
Round 1 (2026-04-26) collapsed the dual-pass dispatch into the
per-language registry + cardinal codegen. Round 2 (2026-04-26 PM)
...
2026-04-26** — the relevant content moved into "Current architecture"
above. Layers 2 and 3 are still proposed.

### Layer 1 — Per-language registry — LANDED 2026-04-26
...
1. Re-run `uv run python scripts/codegen_num2lang.py --output crates/batchalign-chat-ops/data/num2lang.json`.
...
4. Run `cargo test -p batchalign-chat-ops --lib`; the
...
- [Malayalam Language Support](../reference/languages/malayalam.md)
  ...
  after the 2026-04-26 fix.
- `crates/batchalign-chat-ops/CLAUDE.md` — `asr_postprocess/` module
  map; references `num2text.rs` for number expansion specifically.
- `crates/batchalign-app/CLAUDE.md` — Python boundary policy
