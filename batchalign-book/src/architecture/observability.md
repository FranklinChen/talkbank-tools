...
**Status:** Current
**Last modified:** 2026-04-14 15:15 EDT
...
### Submit-path retries (2026-04-14)

`BatchalignClient::submit_job` (`crates/batchalign-cli/src/client.rs`) goes
...
- **Retry class:** transient `reqwest::Error::is_connect()` or `is_timeout()`
  only. These cover the daemon's accept-gap class documented in
  [Server Known Issues](server-known-issues.md#transient-accept-gap-at-job-finalization).
...
`crates/batchalign-cli/src/client.rs` (`submit_job`, `request_with_retry`,
`read_http_error_detail`, constants `RETRY_ATTEMPTS`/`RETRY_BACKOFF`),
`crates/batchalign-app/src/routes/jobs/mod.rs` (`submit_job` handler),
`crates/batchalign-app/src/routes/mod.rs` (`RequestBodyLimitLayer`),
`crates/batchalign-app/src/types/config/server.rs`
...
**Per-language tagger (2026-04-14).** The drain loop in `infer_batched.rs`
...
The fix lives in `crates/batchalign-app/src/morphosyntax/worker.rs::infer_batch`:
...
in `crates/batchalign-app/src/runner/util/batch_progress.rs`.
...
`crates/batchalign-app/src/morphosyntax/worker.rs`,
`crates/batchalign-app/src/runner/dispatch/infer_batched.rs`,
`crates/batchalign-app/src/runner/util/batch_progress.rs`.

### Job status authority: local store vs Temporal (2026-04-14)
...
**Fix landed 2026-04-14:** a Temporal reconciler
(`crates/batchalign-app/src/temporal_reconciler.rs`) that pushes Temporal's
...
Diagram verified against:
`crates/batchalign-app/src/temporal_reconciler.rs`,
`crates/batchalign-app/src/temporal_backend.rs`,
`crates/batchalign-app/src/store/queries/mod.rs`.
...
would always return `[]`. The 2026-04-14 fix is a load-bearing prerequisite
for heartbeat-gap diagnostics.
